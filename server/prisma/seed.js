...
async function main() {
    ...
    // ---------------------------------------------------------
    const defaultPassword = await bcryptjs_1.default.hash('Admin@123', 10);
    const wardenPassword = await bcryptjs_1.default.hash('Warden@123', 10);
    const acctPassword = await bcryptjs_1.default.hash('Accountant@123', 10);
    const secPassword = await bcryptjs_1.default.hash('Security@123', 10);
    const messPassword = await bcryptjs_1.default.hash('Mess@123', 10);
    const maintPassword = await bcryptjs_1.default.hash('Maintenance@123', 10);
    const studentPassword = await bcryptjs_1.default.hash('Student@123', 10);
    const parentPassword = await bcryptjs_1.default.hash('Parent@123', 10);
    ...
    for (let i = 0; i < createdStudents.length; i++) {
        ...
        const availableBed = allCreatedBeds.find((b) => b.hostelId === targetHostel.id && b.status === 'AVAILABLE');
        if (availableBed && i < 40) {
            ...
            const occupiedBedsInRoom = allCreatedBeds.filter((b) => b.roomId === room.id && b.status === 'OCCUPIED').length;
